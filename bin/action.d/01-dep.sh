...
build_pkg(){
  ...
  if [[ ! -f $DEP_REVISION ]];then
    ...
    LIB_DIRS=$(find $DEP_INSTALL_DIR -name lib -type d|xargs -d "\n"|sed -e 's/ /:/g')
    INCLUDE_DIRS=$(find $DEP_INSTALL_DIR -name include -type d|xargs -d "\n"|sed -e 's/ /:/g')
    export LIBRARY_PATH=$LIB_DIRS
    export C_INCLUDE_PATH=$INCLUDE_DIRS
